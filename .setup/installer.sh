...
install_completions() {
    ...
    if [ "$(command -v poetry)" ] && [ ! -f "$ZFUNC_DIR/_poetry" ]; then
        echo -n "poetry, "
        poetry completions zsh >"$ZFUNC_DIR/_poetry"
    fi

    if [ ! -f "$ZFUNC_DIR/_docker-compose" ]; then
...
install_tools_from_curl() {
    ...
    # vim-plug
    if [ ! -f "$LOCAL_SHARE/nvim/site/autoload/plug.vim" ]; then
...
install_i3() {
    ...
    [ "$(command -v dnf)" ] || return
    if [ "$(fedora_version)" -lt 32 ]; then # i3-gaps from this repo is not yet built for fedora 32
